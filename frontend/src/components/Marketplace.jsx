...
const Marketplace = () => {
  ...
  const [mode, setMode] = useState('SELL'); // Mode: SELL or BUY
  ...
  const fetchListings = async () => {
    try {
      const endpoint =
        mode === 'SELL' ? '/marketplace/sell-listings' : '/marketplace/buy-listings';
        ...
        {listings.length === 0 ? (
          ...
          >
            {listings.map((listing, index) => {
              const co2eOffsets = parseFloat(listing.co2e_offsets.replace(' tons', ''));
              const credits = parseInt(listing.credits, 10);
              const percentage = Math.min((credits / co2eOffsets) * 100, 100).toFixed(0);
              const excessCredits = credits > co2eOffsets ? credits - co2eOffsets : 0;
              const requiredCredits = co2eOffsets > credits ? co2eOffsets - credits : 0;
              ...
              return (
                ...
                >
                  ...
                  <p className="text-green-100">
                    <strong>Credits:</strong> {formatNumberWithCommas(listing.credits)}
                  </p>
                  <p className="text-green-100">
                  ...
                  <p className="text-green-100">
                    ...
                    {calculateCO2Efficiency(listing.co2e_offsets, listing.credits)} CO2 per credit
                  </p>
                  <div className="flex items-center mt-2">
                    ...
                    <span className="ml-2 text-white text-sm font-semibold">{percentage}%</span>
                  </div>
        ...
        {isChatOpen && selectedListing && (
          <div className="fixed inset-0 bg-black/70 flex items-center justify-center">
            <div className="bg-gradient-to-br from-green-900 via-green-800 to-green-700 text-white p-8 rounded-xl shadow-2xl max-w-lg w-full">
              <div className="flex justify-between items-center mb-6">
                <h2 className="text-xl font-bold">Chat with {selectedListing.company_name}</h2>
                <X
